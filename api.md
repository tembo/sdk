...
List Tembo’s library with the caller’s connection status:

```ts
const servers = await client.mcpConnections.suggested();
```

Requires `GET /v1/mcp-connections/suggested` on the API deployment. Use `mcpConnections.create` for a custom remote MCP URL and `authorize` to resume OAuth.
