...
test('session extensions preserve authenticated paths, bodies and resume headers',async()=>{
  const requests=[];
  const client=new Tembo({apiKey:'test',maxRetries:0,fetch:async(input,init)=>{
    const request=new Request(input,init);requests.push({url:request.url,method:request.method,headers:request.headers,body:await request.text()});
    return request.url.endsWith('/stream') ? new Response('id: 42\nevent: activity\ndata: {}\n\n',{headers:{'content-type':'text/event-stream'}}) : Response.json({items:[]});
  }});
  const routine={id:'00000000-0000-4000-8000-000000000001',requestId:'00000000-0000-4000-8000-000000000002',expectedRevision:0,reporting:'useful',name:'Briefing',prompt:'Use this session’s files',cron:'0 9 * * 1-5',timezone:'America/New_York',enabled:true};
  await client.sessions.automations.create('session/one',routine);
  await client.sessions.automations.update('session/one','schedule/one',{...routine,expectedRevision:1,enabled:false});
  ...
  await client.sessions.automations.delete('session/one','schedule/one',{requestId:'stable-delete',expectedRevision:2});
